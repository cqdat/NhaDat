...
namespace bds.Models
{
    public class ImageResizer
    {
        ...
        public bool ResizeImage(string sourceFile, string destFile, int MyWidth, int MyHeight)
        {
            try
            {
                ...
                if (resizer.Resize(sourceFile, destFile))
                {
                    return true;
                }
                else return false;
        ...
        public string ConvertExtention(string filename, string extention)
        {
            var filenamelist = filename.Split('.');
            if (filenamelist.Length > 1)
            {
                filenamelist[filenamelist.Length - 1] = extention;
